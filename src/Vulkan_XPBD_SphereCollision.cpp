...
struct MeshPushConstants {
    ...
    float radius;
    float amplitude;
    glm::vec4 sphereCenter; //xyz represents sphere center, and w represents speed
    glm::vec4 sphereCenterOriginal;
};
...
static std::vector<SphereNode> generateSphereNodes(glm::vec3 center, float radius, uint32_t step) {
    ...
    for (uint32_t i = 1; i < height - 1; i++) {
        for (uint32_t j = 0; j < width; j++) {
			theta = (float)i / (height - 1) * glm::pi<float>();
			phi = (float)j / (width - 1) * glm::pi<float>() * 2;

			float posX = center.x + radius * sinf(theta) * cosf(phi);
			float posY = center.y - radius * cosf(theta);
			float posZ = center.z + radius * sinf(theta) * sinf(phi);
            node.pos = glm::vec4(posX, posY, posZ, 0.0f);

            //node.color = glm::vec4(i / float(height), 0.0f, 0.0f, 0.0f);
            node.color = glm::vec4(glm::pow(i / float(width), 2.2f));
    ...
    node.pos = glm::vec4(center.x, center.y + radius, center.z, 0.0f);
    node.color = glm::vec4(0.0f);
    node.normal = glm::vec4(1.0f);
    node.isFixed = 1.0f;
	nodes.push_back(node);

	node.pos = glm::vec4(center.x, center.y - radius, center.z, 0.0f);
	node.color = glm::vec4(0.0f);
	node.normal = glm::vec4(0.0f);
	node.isFixed = 1.0f;
	nodes.push_back(node);
...
static std::vector<uint32_t> generateSphereIndices(uint32_t step) {
    std::vector<uint32_t> indices;    
	int width = step * 2;
	int height = step;
...
//const std::vector<SphereNode> sphereNodes = generateSphereNodes(sphereCenter, 0.375f, 16);
const std::vector<SphereNode> sphereNodes = generateSphereNodes(glm::vec3(0.0f), 0.375f * 0.98f, 16);
const std::vector<uint32_t> sphereIndices = generateSphereIndices(16);
...
private:
    ...
    void createSphereBuffer() {
        VkDeviceSize vertexBufferSize = sphereNodes.size();
        std::vector<SphereNode> sphereNodesLocal(vertexBufferSize);

        VkDeviceSize bufferSize = sizeof(SphereNode) * sphereNodesLocal.size();
        ...
        void* data;
        vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, sphereNodes.data(), (size_t)bufferSize);
        vkUnmapMemory(device, stagingBufferMemory);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(
                ...
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                sphereBuffers[i],
                sphereBuffersMemory[i]
            );
            copyBuffer(stagingBuffer, sphereBuffers[i], bufferSize);
        }
        vkDestroyBuffer(device, stagingBuffer, nullptr);
        vkFreeMemory(device, stagingBufferMemory, nullptr);
    }
    ...
    void createSphereIndexBuffer() {
        ...
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, sphereIndexBuffer, sphereIndexBufferMemory);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &staticModel);
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
        
		VkBuffer sphereSSBuffers[] = { sphereBuffers[currentFrame] };
		VkDeviceSize sphereOffsets[] = { 0 };
		vkCmdBindVertexBuffers(commandBuffer, 0, 1, sphereSSBuffers, sphereOffsets);
		vkCmdBindIndexBuffer(commandBuffer, sphereIndexBuffer, 0, VK_INDEX_TYPE_UINT32);
        glm::mat4 movingModel = glm::mat4(1.0f);
		movingModel = glm::translate(movingModel, sphereCenter);
        vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &movingModel);
